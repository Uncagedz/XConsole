...
| Resolved in Phase 1 hardening | API routes were broadly exempted from top-level authentication | Only `/api/health` and CORS preflight remain public; all other legacy routes require Basic/session auth or a dedicated 32+ character service bearer token | Keep the authenticated gateway in front and rotate the legacy service token with other deployment secrets |
